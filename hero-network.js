(() => {
  ...
  function createNodes() {
    const count =
      width < 700 ? settings.mobileNodes : settings.desktopNodes;
  ...
  function drawConnections() {
    for (let i = 0; i < nodes.length; i += 1) {
      for (let j = i + 1; j < nodes.length; j += 1) {
        ...
        const opacity =
          (1 - distance / settings.maxDistance) * 0.18;
  ...
  function drawNode(node, time) {
    const pulse =
      0.78 + Math.sin(time * 0.0016 + node.phase) * 0.2;
    ...
    glow.addColorStop(0, "rgba(145, 132, 255, 0.72)");
    ...
    ctx.fillStyle = "rgba(150, 140, 255, 0.82)";
    ctx.fill();
  }
